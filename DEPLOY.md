...
> **自动化备份脚本**：项目自带 `bakdata/backup.sh` 脚本，但推荐直接用 Docker exec 方式（上面的 cron 示例），因为 MySQL 在容器内，端口映射为 3307，且 root 密码在 `.env` 中。完整数据库备份命令：
>
> ```bash
> TIMESTAMP=$(date +%Y%m%d_%H%M%S)
> docker exec budgetpilot-mysql-1 mysqldump -u root -p${DB_ROOT_PASS} --routines --single-transaction budgetpilot > bakdata/backup_${TIMESTAMP}.sql
> ```
>
> 此方式会导出全部 13 张表，数据完整。
...
> **说明**：Redis 是**增强型依赖**（optional），不是硬依赖。Redis 不可用时，系统会优雅降级：登录限流、报表缓存、预警去重等功能跳过，核心功能（登录、查询、交易、报表）不受影响。Redis 恢复后自动恢复。
