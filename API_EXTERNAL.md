...
- **认证方式**: `X-Api-Key` 请求头
- **时区**: `Asia/Shanghai` (UTC+8)，所有日期字段按此时区解释
- **金额精度**: `DECIMAL(10,2)`，保留 2 位小数
- **响应格式**: 所有接口返回统一包装
...
> **HTTP 状态码说明**: 所有业务错误（如余额不足、参数校验失败等）**HTTP 状态码始终返回 200**，通过响应体中的 `code` 字段判断成功或失败。仅在接口路径不存在时返回 HTTP 404。外部系统应以 `code` 为准，不要依赖 HTTP 状态码判断业务结果。
...
| 项目 | 说明 |
|------|------|
| **首次获取** | 联系 BudgetPilot 系统管理员，通过「设置」页面或 `/auth/api-key/generate` 接口生成 |
| **每个用户 Key 数量** | 仅 1 个 |
| **Key 轮换** | 重新生成会**立即覆盖**旧 Key，旧 Key 即刻失效。建议在业务低峰期操作 |
| **Key 泄露处理** | 立即重新生成新 Key，旧 Key 自动失效 |
...
> 基础币种为 `CNY`，非 CNY 交易会按当前汇率折算为 `amountBase` 字段存储在响应中返回。

### 3. 幂等性保证

创建交易接口内置**防重复提交**机制：系统对 `(用户 + 类型 + 金额 + 账户 + 分类 + 日期 + 备注)` 计算 hash，**3 秒内**相同的请求会被拦截，返回错误码 `10004`。

**建议**：外部系统网络超时时，可以安全重试；如果收到 `10004` 说明交易已成功，可通过查询接口确认。

### 4. 频率限制

当前**暂无频率限制**。后续如有限流策略，会提前通知。

### 5. 测试环境

当前**没有独立的测试/沙箱环境**。建议对接时：
1. 先在测试用户下创建少量数据，验证接口调用链路
2. 验证通过后切换为正式用户
3. 测试数据可通过 `DELETE /api/v1/transactions/{id}` 和 `DELETE /api/v1/accounts/{id}` 清理

### 6. 超时与排查

请求超时后，交易可能已创建成功（服务端已提交但响应未返回）。建议：
1. 等待 3 秒后重试，会收到 `10004` 重复提交错误，说明交易已存在
2. 通过查询交易列表接口，用 `accountId + transactionDate + amount` 组合确认交易是否存在

---

## 快速开始

### 1. 生成 API Key（需要已有 Key）
...
| `sort` | string | 否 | 排序字段，默认 `transaction_date,desc`（交易日期倒序） |
...
| `10003` | 系统内部异常 | 服务端错误 |
| `10004` | 重复提交 | 账户/商户名称重复，或 3 秒内重复创建相同交易（幂等拦截） |
| `20001` | 账户不存在 | |
| `20002` | 账户已停用 | |
