...
#### 测试用例 TX-06-01：交易列表商户列显示

| 步骤 | 操作 | 预期结果 |
|------|------|----------|
| 1 | 查看交易列表 | "商户"列显示在分类和金额之间 |
| 2 | 查看有商户的交易 | 显示商户名称，颜色为商户自定义颜色或#666 |
| 3 | 查看无商户的交易 | 商户列显示"-" |
| 4 | 查看通过API创建但未填商户的交易 | 商户列显示"-"，可后续补填 |

**验证点**：
- 商户列宽度120px
- 有商户时显示带颜色的名称，无商户显示"-"

#### 测试用例 TX-06-02：编辑交易补填商户（已有商户）

| 步骤 | 操作 | 预期结果 |
|------|------|----------|
| 1 | 编辑一条无商户的已有交易 | 表单中商户字段可选 |
| 2 | 在商户下拉中选择已有商户"星巴克" | 商户选中 |
| 3 | 点击保存 | 提示"更新成功" |
| 4 | 查看交易列表 | 商户列显示"星巴克" |
...
- 数据库 `merchant_id` 已更新
- 商户 `usage_count` +1
- 交易 `is_confirmed` 状态不变

#### 测试用例 TX-06-03：编辑交易补填商户（自动创建新商户）

| 步骤 | 操作 | 预期结果 |
|------|----------|
| 1 | 编辑一条无商户的已有交易 | 表单中商户字段可选 |
| 2 | 在商户输入框中输入不存在的商户名"新开小店" | 显示"创建新商户"选项 |
| 3 | 选择创建新商户 | 商户名填入 |
| 4 | 点击保存 | 提示"更新成功"，新商户创建，交易关联该商户 |

**验证点**：
- `t_merchant` 表新增记录
- 交易 `merchant_id` 指向新商户
- 仅支出/收入类型交易显示此功能（转账不显示）

#### 测试用例 TX-06-04：收入交易关联商户

| 步骤 | 操作 | 预期结果 |
|------|------|----------|
| 1 | 新增收入交易 | 商户字段可见（不仅限于支出） |
| 2 | 选择商户"公司A" | 商户选中 |
| 3 | 提交 | 创建成功，商户关联到收入交易 |

**验证点**：
- 收入类型交易也支持商户字段
- `v-if="form.type !== 3"` 条件正确（转账除外）
...
#### 测试用例 RC-03：创建支出周期规则关联商户

| 步骤 | 操作 | 预期结果 |
|------|------|----------|
| 1 | 进入周期交易管理，点击"新增规则" | 弹出创建表单 |
| 2 | 类型选择"支出" | 商户下拉框出现 |
| 3 | 在商户下拉中选择已有商户"房东A" | 商户选中 |
| 4 | 填写其他必填字段后提交 | 创建成功，规则关联商户 |

**验证点**：
- 数据库 `t_recurring_rule.merchant_id` 已设置
- 收入类型不显示商户字段（`v-if="createForm.type === 1"`）
- 商户下拉仅显示已启用的商户（`isActive: true`）

#### 测试用例 RC-04：周期规则执行时商户传递

| 步骤 | 操作 | 预期结果 |
|------|------|----------|
| 1 | 创建关联商户的周期规则 | 规则保存成功 |
| 2 | 手动执行规则（POST /api/v1/recurring-rules/{id}/execute） | 返回 code=0 |
| 3 | 查看新生成的交易 | 交易的 `merchantId` 等于规则的商户ID |
| 4 | 查看交易列表 | 商户列显示规则关联的商户名 |

**验证点**：
- 交易 `merchant_id` 继承自规则
- 商户 `usage_count` +1
- 交易 metadata 包含 `source: "recurring"`

#### 测试用例 RC-05：周期规则不自动创建商户

| 步骤 | 操作 | 预期结果 |
|------|------|----------|
| 1 | 创建周期规则，类型支出 | 商户下拉框出现 |
| 2 | 在商户输入框中输入不存在的商户名 | 无"创建新商户"选项 |
| 3 | 只能从已有商户中选择 | 只能选择已存在的商户 |

**验证点**：
- 周期规则商户字段仅支持 `filterable` 搜索已有商户
- 不提供自动创建商户功能

#### 测试用例 RC-06：编辑周期规则更换商户

| 步骤 | 操作 | 预期结果 |
|------|------|----------|
| 1 | 编辑已关联商户的规则 | 表单中商户字段显示当前商户 |
| 2 | 更换为另一商户 | 商户更新 |
| 3 | 点击保存 | 更新成功 |
| 4 | 手动执行规则 | 新交易关联新商户 |
...
| BUG-013 | API创建的交易无商户无法后续补填 | 编辑交易时商户字段可见，可选择已有商户或自动创建新商户 |
| BUG-014 | 周期规则无商户字段 | 支出类型周期规则可关联已有商户，执行时传递给交易 |
